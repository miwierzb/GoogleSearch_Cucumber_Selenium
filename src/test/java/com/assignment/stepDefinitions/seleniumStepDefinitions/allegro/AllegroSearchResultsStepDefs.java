...
public class AllegroSearchResultsStepDefs {
    ...
    public void verifyIfResultsAreCorrectlyFilteredForChosenParameters(String filterFrom, String filterTo, String option){
        logger().info("STEP: I should see correctly filtered results for " + filterFrom + " gb to " + filterTo + " gb and " + option);
            ...
            switch(sorting){
                ...
                case "price_ascending":
                    if (Double.parseDouble(itemPrices.get(i-1).split("( zł)")[0].replace(",", ".").replace(" ", ""))
                            - Double.parseDouble(itemPrices.get(i).split("( zł)")[0].replace(",", ".").replace(" ", "")) > 0){
                        sorted = false;
                        logger().info("Price is not sorted - element " + (i-1) + ": " + itemPrices.get(i-1) + " is higher that element " + i + ": " + itemPrices.get(i));
                    }
                    break;
            }
        }
